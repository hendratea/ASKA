var table_referensi_golongan;

function table_refGolongan() {
  var url;
  url = baseURL + "get_referensi_golongan";
  table_referensi_golongan = $("#dt_referensi_golongan").DataTable({
    destroy: true,
    iDisplayLength: 10,
    processing: true,
    searching: true,
...
function btnSaveData() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = baseURL + "save_referensi_golongan";
        ...
        success: function (data) {
          if (data["statusInsertToDb"] == 1) {
            Swal.fire({
              ...
              table_referensi_golongan.ajax.reload();
              $("#ID,#status,#aptln").val("");
            });
...
function btnUpdateData() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var url = baseURL + "update_referensi_golongan";
      var postData = $.param({
        ajaxID: $("#ID").val(),
        ajaxStatus: $("#status").val(),
        ajaxAptln: $("#aptln").val(),
...
function confirmDeleteData(rID) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = baseURL + "delete_referensi_golongan";
      var postData = $.param({
        ajaxID: rID,
      });
      $.ajax({
...
function confirmUpdateData(rID) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var url = baseURL + "get_forupdate_referensi_golongan";
      var datapost = {
        ajaxID: rID,
      };
...
$("#btnSubmit").click(function () {
  ...
  statusValidAptln = validationForm("#aptln", "#boxIconAptln", "#msgErrAptln", "aptln");

  validationFormValid.push(statusValidStatus, statusValidAptln);
