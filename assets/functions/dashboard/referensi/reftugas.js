var table_referensi_tugas;

function table_refTugas() {
  var url;
  url = baseURL + "get_referensi_tugas";
  table_referensi_tugas = $("#dt_referensi_tugas").DataTable({
    destroy: true,
    iDisplayLength: 10,
    processing: true,
    searching: true,
...
function btnSaveData() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = baseURL + "save_referensi_tugas";
        ...
        success: function (data) {
          if (data["statusInsertToDb"] == 1) {
            Swal.fire({
              ...
              table_referensi_tugas.ajax.reload();
              $("#ID,#status").val("");
            });
...
function btnUpdateData() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var url = baseURL + "update_referensi_tugas";
      var postData = $.param({
        ajaxID: $("#ID").val(),
        ajaxStatus: $("#status").val(),
      });
...
function confirmDeleteData(rID) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = baseURL + "delete_referensi_tugas";
      var postData = $.param({
        ajaxID: rID,
      });
      $.ajax({
...
function confirmUpdateData(rID) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var url = baseURL + "get_forupdate_referensi_tugas";
      var datapost = {
        ajaxID: rID,
      };
