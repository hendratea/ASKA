var table_referensi_fungsi;

function table_refFungsi() {
  var url;
  url = baseURL + "get_referensi_fungsi";
  table_referensi_fungsi = $("#dt_referensi_fungsi").DataTable({
    destroy: true,
    iDisplayLength: 10,
    processing: true,
    searching: true,
...
function btnSaveData() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = baseURL + "save_referensi_fungsi";
        ...
        success: function (data) {
          if (data["statusInsertToDb"] == 1) {
            Swal.fire({
              ...
              table_referensi_fungsi.ajax.reload();
              $("#ID,#status").val("");
            });
...
function btnUpdateData() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var url = baseURL + "update_referensi_fungsi";
      var postData = $.param({
        ajaxID: $("#ID").val(),
        ajaxStatus: $("#status").val(),
      });
...
function confirmDeleteData(rID) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = baseURL + "delete_referensi_fungsi";
      var postData = $.param({
        ajaxID: rID,
      });
      $.ajax({
...
function confirmUpdateData(rID) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var url = baseURL + "get_forupdate_referensi_fungsi";
      var datapost = {
        ajaxID: rID,
      };
