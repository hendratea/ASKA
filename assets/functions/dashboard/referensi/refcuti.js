var table_referensi_cuti;

function table_refCuti() {
  var url;
  url = baseURL + "get_referensi_cuti";
  table_referensi_cuti = $("#dt_referensi_cuti").DataTable({
    destroy: true,
    iDisplayLength: 10,
    processing: true,
    searching: true,
...
function btnSaveData() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = baseURL + "save_referensi_cuti";
        ...
        success: function (data) {
          if (data["statusInsertToDb"] == 1) {
            Swal.fire({
              ...
              table_referensi_cuti.ajax.reload();
              $("#ID,#status,#jatah").val("");
            });
...
function btnUpdateData() {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var url = baseURL + "update_referensi_cuti";
      var postData = $.param({
        ajaxID: $("#ID").val(),
        ajaxStatus: $("#status").val(),
        ajaxJatah: $("#jatah").val(),
...
function confirmDeleteData(rID) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      var url = baseURL + "delete_referensi_cuti";
      var postData = $.param({
        ajaxID: rID,
      });
      $.ajax({
...
function confirmUpdateData(rID) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      var url = baseURL + "get_forupdate_referensi_cuti";
      var datapost = {
        ajaxID: rID,
      };
...
$("#btnSubmit").click(function () {
  validationFormValid = [];
  statusValidID = validationForm("#ID", "#boxIconID", "#msgErrID", "ID");
  statusValidStatus = validationForm("#status", "#boxIconStatus", "#msgErrStatus", "status");
  statusValidJatah = validationForm("#jatah", "#boxIconJatah", "#msgErrJatah", "jatah");

  validationFormValid.push(statusValidID, statusValidStatus, statusValidJatah);
